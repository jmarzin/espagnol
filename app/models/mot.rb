class Mot < ActiveRecord::Base
  ...
  validates :espagnol, presence: {message: 'La traduction espagnolne est obligatoire'}

  def self.recherche(recherche)
    if recherche
      Mot.where('mot_directeur || francais || espagnol like ?','%'+recherche+'%')
    else
      Mot.all
    end
  end
  ...
  def update(mot_params)
    ...
    self.espagnol = mot_params['espagnol']
  ...
  def self.sauve(user_id)
    ...
    Mot.all.order(:mot_directeur).each do |v|
      IO.write(liste,\
        ...
        v.scores_mots.where(user_id: user_id).first.compteur.to_s+",\""+v.espagnol+"\"],\n",liste.size)
    end
    IO.write(liste,"]\n",liste.size-2)
    true
  end
  ...
  def self.api_v1
    ...
    Mot.order(:category_id, :mot_directeur, :francais).each do |mot|
      liste << [mot.category.numero, mot.francais, mot.mot_directeur, mot.espagnol]
    end
    liste
  end
  ...
  def self.api_v2
    liste = []
    Mot.order(:category_id, :mot_directeur, :francais).each do |mot|
      liste << [mot.id, mot.category_id, mot.francais, mot.mot_directeur, mot.espagnol]
    end
    liste
  end
